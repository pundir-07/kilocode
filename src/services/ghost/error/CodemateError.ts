...
import { CODEMATE_ACCOUNT_AUTH_ERROR_MESSAGE } from "../../../shared/CodemateAccount";

export enum CodemateErrorType {
  Auth = "auth",
  Network = "network",
  RateLimit = "rateLimit",
  Balance = "balance",
...
export class CodemateError extends Error {
  readonly title = "CodemateError";
  ...
  /**
   * Parses a stringified error into a CodemateError instance.
   */
  static parse(errorStr?: string, modelId?: string): CodemateError | undefined {
    if (!errorStr || typeof errorStr !== "string") {
      return undefined;
    }
    return CodemateError.transform(errorStr, modelId);
  }

  /**
   * Transforms any object into a CodemateError instance.
   * Always returns a CodemateError, even if the input is not a valid error object.
   */
  static transform(
    error: any,
    modelId?: string,
    ...
    try {
      return new CodemateError(JSON.parse(error), modelId, providerId);
    } catch {
      return new CodemateError(error, modelId, providerId);
    }
  }

  public isErrorType(type: CodemateErrorType): boolean {
    return CodemateError.getErrorType(this) === type;
  }
   ...
   */
  static getErrorType(err: CodemateError): CodemateErrorType | undefined {
    ...
    if (
      ...
      return CodemateErrorType.Balance;
    }

    // Check auth errors
    if (code === "ERR_BAD_REQUEST" || status === 401) {
      return CodemateErrorType.Auth;
    }

    // Check for auth message (only if message exists)
    const message = err.message;
    if (message?.includes(CODEMATE_ACCOUNT_AUTH_ERROR_MESSAGE)) {
      return CodemateErrorType.Auth;
    }
    ...
    if (message) {
      ...
      if (RATE_LIMIT_PATTERNS.some((pattern) => pattern.test(lowerMessage))) {
        return CodemateErrorType.RateLimit;
      }
    }
