...
export async function activate(context: vscode.ExtensionContext) {
	...
	// kilocode_change start
	if (!context.globalState.get("firstInstallCompleted")) {
		outputChannel.appendLine("First installation detected, opening CodeMate sidebar!")
		try {
			await vscode.commands.executeCommand("codemate.SidebarProvider.focus")

			outputChannel.appendLine("Opening CodeMate walkthrough")

			// this can crash, see:
			// https://discord.com/channels/1349288496988160052/1395865796026040470
			await vscode.commands.executeCommand(
				"workbench.action.openWalkthrough",
				"AyushSinghal.codemate#codemateWalkthrough",
				false,
			)
		} catch (error) {
