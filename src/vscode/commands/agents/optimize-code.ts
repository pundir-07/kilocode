...
export function registerOptimizeCodeCommand(context: vscode.ExtensionContext, commandName: string) {
   const command = vscode.commands.registerCommand(commandName, async (...args) => {
      ...
      try {
         ...
         // Make API call to get optimization suggestions
         ...
         vscode.window.showInformationMessage("Optimization suggestions ready!")
      } catch (error) {
