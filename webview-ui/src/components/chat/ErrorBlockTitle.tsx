...
import {CodemateError,CodemateErrorType} from "../../../../src/services/ghost/error/CodemateError"
...
export const ErrorBlockTitle = ({
	...
	const title = (() => {
		...
		const details = { title: "Working...", classNames: ["font-bold"] }
		...
		if (apiReqCancelReason === "user_cancelled") {
			...
			const codemateError = CodemateError.parse(apiRequestFailedMessage)
			const titleText = codemateError?.isErrorType(CodemateErrorType.Balance)
